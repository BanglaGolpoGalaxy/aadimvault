  ...
  try {
    ...
    const products = result.length && result[0].values.length
      ? result[0].values.map(row => ({
          ...
          image: row[8], images: row[9] ? JSON.parse(row[9]) : [], pdf: row[10],
          auction_end: row[11], story_en: row[12], story_local: row[13],
          local_lang: row[14], created_at: row[15]
        }))
      : [];
    res.json(products);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      image: p[8], images: p[9] ? JSON.parse(p[9]) : [], pdf: p[10],
      auction_end: p[11], story_en: p[12], story_local: p[13],
      local_lang: p[14], created_at: p[15]
    });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error.' }); }
});
...
router.post('/', auth, upload.fields([{ name: 'images', maxCount: 5 }, { name: 'pdf', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    const db = await getDB();
    db.run(`INSERT INTO products (user_id, title, description, price, for_sale, display_only, auction, image, images, pdf, auction_end, story_en, story_local, local_lang) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [req.user.id, title, description||null, parseFloat(price)||0, for_sale==='true'||for_sale==='1'?1:0, display_only==='true'||display_only==='1'?1:0, auction==='true'||auction==='1'?1:0, primaryImage, JSON.stringify(imagePaths), pdfPath, auction_end||null, story_en||null, story_local||null, local_lang||'en']);
    saveDB();
...
router.put('/:id', auth, upload.fields([{ name: 'images', maxCount: 5 }, { name: 'pdf', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    let params = [title, description||null, parseFloat(price)||0, for_sale==='true'||for_sale==='1'?1:0, display_only==='true'||display_only==='1'?1:0, auction==='true'||auction==='1'?1:0, story_en||null, story_local||null, local_lang||'en', auction_end||null];
